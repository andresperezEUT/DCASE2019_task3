...
params_files = {'gt_eval': os.path.join(params_path.get('gt_files'), 'gt_eval_parametric_Q.csv'),
...
# list of clips segmented as the result of Andres frontend, for me to do predictions on them
gt_eval = pd.read_csv(params_files.get('gt_eval'))
filelist_audio_eval_param = gt_eval.fname.values.tolist()
...
if params_ctrl.get('feat_ext'):
    if params_ctrl.get('pipeline') == 'T_F':
        ...
        n_extracted_eval_param = 0; n_failed_eval_param = 0
        ...
        # vip do feature extraction for dev_param (outcome of Andres approaches)
        ...
        # vip do feature extraction for eval_param (outcome of Andres frontend, to predict for EVAL submission)
        audio_files_eval_param = [f for f in os.listdir(params_path.get('audiopath_eval_param')) if not f.startswith('.')]

        nb_files_eval_param = len(audio_files_eval_param)
        if not os.path.exists(params_path.get('featurepath_eval_param')) or \
                        len(os.listdir(params_path.get('featurepath_eval_param'))) < nb_files_eval_param * 0.8:

            if os.path.exists(params_path.get('featurepath_eval_param')):
                shutil.rmtree(params_path.get('featurepath_eval_param'))
            os.makedirs(params_path.get('featurepath_eval_param'))

            print(
                '\nFeature extraction for EVAL set parametric (outcome of Andres). Features dumped in {}.........................'.
                    format(params_path.get('featurepath_eval_param')))
            for idx, f_name in enumerate(audio_files_eval_param):
                f_path = os.path.join(params_path.get('audiopath_eval_param'), f_name)
                if os.path.isfile(f_path) and f_name.endswith('.wav'):
                    # load entire audio file and modify variable length, if needed
                    y = load_audio_file(f_path, input_fixed_length=params_extract['audio_len_samples'],
                                        params_extract=params_extract)
                    ...
                    # keep record of the lengths, per class, for insight
                    duration_seconds = len(y) / int(params_extract.get('fs'))
                    var_lens_eval_param['overall'].append(duration_seconds)
                    ...
                    # save the T_F rep to a binary file (only the considered length)
                    utils_classif.save_tensor(var=mel_spectrogram,
                                              out_path=os.path.join(params_path.get('featurepath_eval_param'),
                                                                    f_name.replace('.wav', '.data')), suffix='_mel')
                    ...
                    if os.path.isfile(os.path.join(params_path.get('featurepath_eval_param'),
                                                   f_name.replace('.wav', suffix_in + '.data'))):
                        n_extracted_eval_param += 1
                        print('%-22s: [%d/%d] of %s' % (
                        'Extracted eval_param features', (idx + 1), nb_files_eval_param, f_path))
                    else:
                        n_failed_eval_param += 1
                        print('%-22s: [%d/%d] of %s' % (
                            'FAILING to extract eval_param features', (idx + 1), nb_files_eval_param, f_path))
                else:
                    print('%-22s: [%d/%d] of %s' % (
                        'this eval_param audio is in the csv but not in the folder', (idx + 1), nb_files_eval_param,
                        f_path))

            print('n_extracted_eval_param: {0} / {1}'.format(n_extracted_eval_param, nb_files_eval_param))
            print('n_failed_eval_param: {0} / {1}\n'.format(n_failed_eval_param, nb_files_eval_param))

        else:
            print('Eval_param set is already extracted in {}'.format(params_path.get('featurepath_eval_param')))
...
ff_list_eval_param = [filelist_audio_eval_param[i].replace('.wav', suffix_in + '.data') for i in range(len(filelist_audio_eval_param))]
...
print('Number of clips considered as eval_param set (to predict for submission): {0}'.format(len(ff_list_eval_param)))


scalers = [None]
# vip determine the learning setup in evaluation mode, and perform training / val / test
# for kfo in range(1, 5):
for kfo in range(0, 1):
    print('\n=========================================================================================================')
    print('===Processing in evaluation mode...')
    print('=========================================================================================================\n')
    ...
    tr_files2 = [fname for idx, fname in enumerate(ff_list_dev) if splitlist_audio_dev[idx] == splits_tr[2]]

    tr_files = tr_files0 + tr_files1 + tr_files2
    val_files = [fname for idx, fname in enumerate(ff_list_dev) if splitlist_audio_dev[idx] == splits_val[0]]

    te_files = [fname for idx, fname in enumerate(ff_list_eval_param)]

    # SC
    if len(tr_files) + len(val_files) != len(ff_list_dev):
        print('ERROR: You messed up train /val split')
        print('{} is not {}'.format(len(tr_files) + len(val_files), len(ff_list_dev)))
    elif len(te_files) != len(ff_list_eval_param):
        print('ERROR: You messed up in eval management')
    ...
    scalers[kfo] = tr_gen_patch.scaler
...
print('\n====================Starting prediction on eval set with REAL frontend=====================================')
print('====================Starting prediction on eval set with REAL frontend=====================================')
print('====================Starting prediction on eval set with REAL frontend=====================================\n')

data_folder_path = '../data/foa_eval/'
...
# vip load best model (do not just take the last model, but load the
model_f0 = load_model(os.path.join(path_trained_models, params_ctrl.get('output_file') + '_v' +
                                   str(params_ctrl.get('count_trial')) + '_f0.h5'))
    ...
    with open(os.path.join(results_metadata_doa_folder, metadata_file_name), 'r') as f:
        ...
        for i, row in enumerate(reader):
            ...
            if i > 0:
                ...
                # new name, according to process_doa_results.py in eval mode
                ...
                ff_list_eval_param_single_file = [filename.replace('.wav', suffix_in + '.data')]

                # grab scaler from the one I saved in training
                current_scaler = scalers[0]

                # vip: we are in REAL conditions, hence the dataset is 'dev_param' (and not dev). Watch path
                te_param_gen_patch = PatchGeneratorPerFile(feature_dir=params_path.get('featurepath_eval_param'),
                                                           file_list=ff_list_eval_param_single_file,
                                                           params_extract=params_extract,
                                                           suffix_in='_mel',
                                                           floatx=np.float32,
                ...
                # predicting now on the T_F patch level (not on the wav clip-level)
                preds_patch_list = model_f0.predict(patches_file).tolist()
