...
cutmix_log = False
cutmix_clamp = False
cutmix_alphas = [2, 1, 0.5, 0.2]
output_file = 'crnn_seld_Q_explore_net_tagger_BETA64_cutmix_alpha_single_lam'
...
def change_yaml(fname, count_trial, output_file, model, loss, patch_len, lr, batch_size, cnn_nb_filt, rnn_nb, fc_nb, n_mel, cutmix_alpha):
    """
    ...
    # watch 1 stage cutmix***********************
    data['learn']['stages'] = 1                  #usually 1 or 2. but 0 for enabling mixup_warmup
    data['learn']['cutmix'] = True                    # True False
    data['learn']['cutmix_mode'] = cutmix_mode
    data['learn']['cutmix_alpha'] = cutmix_alpha
    data['learn']['cutmix_log'] = cutmix_log
    data['learn']['cutmix_clamp'] = cutmix_clamp
    data['learn']['cutmix_single_lam'] = True       # original paper is True
                        ...
                        for batch_size in batch_sizes:
                            for cnn_nb_filt in cnn_nb_filts:
                                for rnn_nb in rnn_nbs:
                                    for fc_nb in fc_nbs:
                                        for n_mel in n_mels:
                                            for cutmix_alpha in cutmix_alphas:
